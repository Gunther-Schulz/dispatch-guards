...
source-blob: bb93897155c681394624778d8506d141e9ed0872  (BACKLOG.md)
...
| quantity | count |
|---|---|
| top-level bullets in `BACKLOG.md` | 38 |
| of those, ENTRIES (bold, or led by a grade-shaped word) | 38 |
| of those, non-entry prose bullets (not migrated) | 0 |
| of those, bullets in a section §4 row 1 CUTS | 0 |
| items written to `ITEMS.md` | 31 |
| of those, CLOSURES routed to `ITEMS-DONE.md` instead | 7 |
| entries reported UNCLASSIFIED or AMBIGUOUS (not written) | 0 |
| archive bodies in `ITEMS-DONE.md` (verbatim) | 7 |
| entries routed to the ledger | 0 |
| RESIDUE items booked (`tend`, from no source entry) | 1 |

**Identity:** 38 entries read = 31 written + 7 closed + 0 unclassified — HOLDS. THREE columns, not two: a closure read out of the source carrier is neither written as an item nor unclassified, and folding it into either would make one of those numbers say something it does not.

**Bullet identity:** 38 top-level bullets = 38 entries + 0 prose + 0 cut — HOLDS. This is the identity that makes 'not migrated' visible: every bullet in the source is in exactly one of the three columns, so a bullet the migration simply did not see would show up as a gap in the sum rather than as nothing at all.

**Conservation (§3.1), computed on the produced files:** items 32 (of which 1 residue) + done 7 = 39; baseline 39 + added 0 − compacted 0 = 39. HOLDS. THE RESIDUE IS IN THIS IDENTITY AND NOT IN THE ONE ABOVE, and the split is the point: a residue item is a BODY in the carrier, so conservation must see it, and it comes from no SOURCE ENTRY, so the reconciliation identity must not — folding it in there would answer COULD NOT VERIFY on every migration after this one.
...
| `PARKED` | NEW | 18 |
| `READY` | READY | 13 |
| `(ungraded)` | CLOSED → the done home | 6 |
| `DONE` | CLOSED → the done home | 1 |

## Entries by source section

| section | entries |
|---|---|
| dispatch-guards — backlog | 0 |
| Open | 31 |
| Done | 7 |

## Closures routed to the done home

7 entry/ies. Each body is in `ITEMS-DONE.md` VERBATIM, under the archive heading, preceded by a comment naming its source line range and the rule that routed it.

| line | source grade word | section | rule |
|---|---|---|---|
| `BACKLOG.md:1064-1092` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:1093-1104` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:1105-1122` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:1123-1180` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:1181-1196` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:1197-1204` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:1205-1217` | `DONE` | Done | §3.1: `DONE` is the closed vocabulary's own word — a closure MOVES to the done home; it never enters the open  |
...
**6 tracked file(s)** still name `BACKLOG.md` — one item, its `write-set` and `evidence` naming every one of them:

- `CLAUDE.md`
- `LEDGER.md`
- `dev-notes/dispatch-OBSERVATIONS.md`
