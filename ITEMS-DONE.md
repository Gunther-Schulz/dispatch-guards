...
blocker-moot: this is a skill edit: CLAUDE.md Discipline requires a skill-craft invocation (pre-edit hook enforced) and a skill-craft:release-plugin release with an operator /reload-plugins handoff, which is a release act on the machinery desk's cadence, not a migration act
...
blocker-moot: should the description-derived slug strip a legacy model title prefix, against the directive's literal spec (answered in the ledger before this item closed)
closed-reason: 2026-09-15 wave 2. compute_name_rewrite strips a leading model prefix from the description-fallback source only when it equals the call's validated model; a different model's prefix stays, per the sibling ruling. Verified at the artifact by the dispatcher: bites and bench green in my own run, and the sibling protection mutation-tested — widening the strip to any allowed model word goes RED against a green control. Residual booked as dg-50: the same doubling survives on the NAME-source path
closed-ref: f37862b
